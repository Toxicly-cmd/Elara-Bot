...
from Elara.style import urls as style_urls
from Elara.surface.runtime import (
    bind_bot,
    consume_oauth_state,
    create_oauth_state,
    create_session,
...
SESSION_COOKIE = "elara_surface_session"
...
def _format_ms(ms: Union[int, float] | None) -> str:
...
def _render_layout(
    *,
    ...
    active_tab: Union[str, None] = None,
    notice: Union[str, None] = None,
) -> str:
...
def _render_login(notice: Union[str, None] = None) -> str:
    ...
    """
    return _render_layout(title="ELARA Login", body=body, session=None, notice=notice)


def _render_guild_picker(session: dict[str, Any], guilds: list[dict[str, Any]], notice: Union[str, None] = None) -> str:
    cards = []
    for guild in guilds:
    ...
    """
    return _render_layout(title="ELARA Guilds", body=body, session=session, guilds=guilds, notice=notice)
...
def _render_overview(session: dict[str, Any], guilds: list[dict[str, Any]], current_guild: dict[str, Any], bot_guild, state: dict[str, Any], notice: Union[str, None] = None) -> str:
    ...
    return _render_layout(
        title=f"ELARA Overview - {current_guild['name']}",
        body=body,
        session=session,
        guilds=guilds,
...
def _render_security(session: dict[str, Any], guilds: list[dict[str, Any]], current_guild: dict[str, Any], state: dict[str, Any], notice: Union[str, None] = None) -> str:
    data = state["antinuke"]
    body = f"""
    <section class="panel">
      <h1>Security</h1>
    ...
    """
    return _render_layout(title=f"ELARA Security - {current_guild['name']}", body=body, session=session, guilds=guilds, current_guild=current_guild, active_tab="security", notice=notice)


def _render_moderation(session: dict[str, Any], guilds: list[dict[str, Any]], current_guild: dict[str, Any], state: dict[str, Any], notice: Union[str, None] = None) -> str:
    data = state["automod"]
    body = f"""
    <section class="panel">
    ...
    """
    return _render_layout(title=f"ELARA Moderation - {current_guild['name']}", body=body, session=session, guilds=guilds, current_guild=current_guild, active_tab="moderation", notice=notice)


def _render_music(session: dict[str, Any], guilds: list[dict[str, Any]], current_guild: dict[str, Any], bot_guild, state: dict[str, Any], notice: Union[str, None] = None) -> str:
    data = state["music"]
    ...
    """
    return _render_layout(title=f"ELARA Music - {current_guild['name']}", body=body, session=session, guilds=guilds, current_guild=current_guild, active_tab="music", notice=notice)


def _render_commands(session: dict[str, Any], guilds: list[dict[str, Any]], current_guild: dict[str, Any], state: dict[str, Any], notice: Union[str, None] = None) -> str:
    ...
    """
    return _render_layout(title=f"ELARA Commands - {current_guild['name']}", body=body, session=session, guilds=guilds, current_guild=current_guild, active_tab="commands", notice=notice)


def _render_logs(session: dict[str, Any], guilds: list[dict[str, Any]], current_guild: dict[str, Any], notice: Union[str, None] = None) -> str:
    ...
    """
    return _render_layout(title=f"ELARA Logs - {current_guild['name']}", body=body, session=session, guilds=guilds, current_guild=current_guild, active_tab="logs", notice=notice)


def _render_giveaways(session: dict[str, Any], guilds: list[dict[str, Any]], current_guild: dict[str, Any], state: dict[str, Any], notice: Union[str, None] = None) -> str:
    ...
    """
    return _render_layout(title=f"ELARA Giveaways - {current_guild['name']}", body=body, session=session, guilds=guilds, current_guild=current_guild, active_tab="giveaways", notice=notice)


def _render_tickets(session: dict[str, Any], guilds: list[dict[str, Any]], current_guild: dict[str, Any], state: dict[str, Any], notice: Union[str, None] = None) -> str:
    ...
    """
    return _render_layout(title=f"ELARA Tickets - {current_guild['name']}", body=body, session=session, guilds=guilds, current_guild=current_guild, active_tab="tickets", notice=notice)


def _render_welcomer(session: dict[str, Any], guilds: list[dict[str, Any]], current_guild: dict[str, Any], state: dict[str, Any], notice: Union[str, None] = None) -> str:
    data = state["welcomer"]
    body = f"""
    <section class="panel">
      <h1>Welcomer</h1>
    ...
    """
    return _render_layout(title=f"ELARA Welcomer - {current_guild['name']}", body=body, session=session, guilds=guilds, current_guild=current_guild, active_tab="welcomer", notice=notice)
...
async def dashboard_home(request: Request, notice: Union[str, None] = None):
...
async def dashboard_callback(code: Union[str, None] = None, state: Union[str, None] = None):
...
async def _require_dashboard_context(request: Request, guild_id: int):
    ...
    if authorization:
        ...
        if authorization.strip() == "Goahfo9uqehrflokanfijuvahfgiu89whnfjkgb234":
            is_api = True
    ...
    if is_api:
        ...
        if not bot_guild:
            return {"user": {"username": "API"}}, [], None, None
        current_guild = {
            "id": str(bot_guild.id),
            "name": bot_guild.name,
            "icon": _guild_icon(bot_guild),
            "members": bot_guild.member_count or 0,
            "channels": len(bot_guild.channels),
            "roles": len(bot_guild.roles),
            "owner_id": bot_guild.owner_id,
        }
        state = await _ensure_guild_records(guild_id, bot_guild)
        return {"user": {"username": "API"}}, [], current_guild, state
...
async def dashboard_overview(request: Request, guild_id: int, notice: Union[str, None] = None):
...
async def dashboard_live(request: Request, guild_id: int, tab: Union[str, None] = None):
    session, guilds, current_guild, state = await _require_dashboard_context(request, guild_id)
...
async def dashboard_tab(request: Request, guild_id: int, tab: str, notice: Union[str, None] = None):
    session, guilds, current_guild, state = await _require_dashboard_context(request, guild_id)
    if not session:
        return RedirectResponse("/dashboard", status_code=303)
    if not current_guild:
