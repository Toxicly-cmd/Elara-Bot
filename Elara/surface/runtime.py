...
def get_session(session_id: Union[str, None]) -> dict[str, Any] | None:
    if not session_id:
        return None
    return _sessions.get(session_id)


def destroy_session(session_id: Union[str, None]) -> None:
    if session_id:
...
def consume_oauth_state(state: Union[str, None]) -> bool:
    if not state:
        return False
