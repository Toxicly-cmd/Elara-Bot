...
from Elara.utils import chat_exporter
...
async def ticket_close_action(
    ...
    try:
        ...
        try:
            close_markdown = (
                f"## {bot.emoji.TICKET} Ticket #{str(ticket_data.get('ticket_id')).zfill(4)} Closed\n"
                f"The support ticket has been finalized and archived.\n\n"
                f"**__Information:__**\n"
                f"> **Creator:** <@{ticket_data.get('creator_id')}>\n"
                f"> **Closed By:** {closed_by.mention if closed_by else 'Unknown'}\n"
                f"> **Created:** <t:{int(ticket_data.get('created_at').timestamp())}:F>\n"
                f"> **Closed:** <t:{int(ticket_data.get('closed_at').timestamp())}:F>\n"
                f"> **Topic:** {channel.topic if channel.topic else 'No topic provided'}\n"
...
async def send_ticket_panel_message(ticket_settings_data, bot: AutoShardedBot):
    try:
        ...
        view = _build_layout_with_actions(
            [f"## {bot.emoji.TICKET} {panel_title}", panel_description],
            [CreateTicketButton],
            timeout=None,
        )
