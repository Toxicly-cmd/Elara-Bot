...
from Elara.src.events import on_voice_state_update
...
async def controller_module(bot: AutoShardedBot, data, channel=None):
    ...
    try:
        ...
        async def get_embed():
            embed = discord.Embed(
                title=f"{bot.emoji.MICROPHONE} Voice Controller",
                description=(
                    "Manage your personal voice channel settings using the interface below.\n\n"
                    f"**__Channel Statistics:__**\n"
                    f"> **Channel:** {channel.mention}\n"
                    f"> **User Limit:** `{channel.user_limit if channel.user_limit != 0 else 'Unlimited'}`\n"
                    f"> **Bitrate:** `{int(channel.bitrate/1000)}kbps`\n"
                    f"> **Slowmode:** `{channel.slowmode_delay}s`\n"
                    f"> **NSFW:** `{channel.is_nsfw()}`\n"
                    f"> **Region:** `{channel.rtc_region if channel.rtc_region else 'Automatic'}`\n"
