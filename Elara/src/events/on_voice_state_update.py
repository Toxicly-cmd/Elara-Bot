...
class on_voice_state_update(commands.Cog):
    def __init__(self, bot):
        ...
        self.voice_tracking = {} # (user_id, guild_id): {join_time, muted_time, deafened_time, afk_time, last_state}
    ...
    async def vcrole_module(self, member: discord.Member, before: discord.VoiceState, after: discord.VoiceState):
        try:
            if before.channel == after.channel:
                return
            
            cache_data = cache.vcrole.get(str(member.guild.id))
            if not cache_data or not cache_data.get('role_id'):
                return
            
            role = member.guild.get_role(cache_data.get('role_id'))
            if not role:
                return
            
            if not before.channel and after.channel: # Join
                if role not in member.roles:
                    await member.add_roles(role, reason="Voice Role (Joined VC)")
            elif before.channel and not after.channel: # Leave
                if role in member.roles:
                    await member.remove_roles(role, reason="Voice Role (Left VC)")
        except Exception as e:
            logger.error(f"Error in vcrole_module: {e}")
            ...
            for voice_channel in guild.voice_channels:
                for member in voice_channel.members:
                    if not member.bot:
                        ...
                        if key not in self.voice_tracking:
                            self.voice_tracking[key] = {
                                'join_time': now,
                                'last_update': now,
                                'muted': member.voice.self_mute or member.voice.mute,
                                'deafened': member.voice.self_deaf or member.voice.deaf,
                                'afk': member.voice.afk
                            }
                            count += 1
        logger.info(f"Initialized voice tracking for {count} members across {len(self.bot.guilds)} guilds")
    ...
    async def on_voice_state_update(self, member: discord.Member, before: discord.VoiceState, after: discord.VoiceState):
        ...
        try:
            ...
            pass
        try:
            asyncio.create_task(self.vcrole_module(member,before,after))
        except Exception as e:
            pass
        ...
        try:
            ...
            # Handle state initialization or update
            if after.channel:
                if key not in self.voice_tracking:
                    # User was either already in VC or just joined
                    self.voice_tracking[key] = {
                        'join_time': now,
                        'last_update': now,
                    ...
                    # logger.debug(f"Initialized voice tracking for {member.name} in {member.guild.name}")
                else:
                    # User was already tracked, calculate delta and update state
                    data = self.voice_tracking[key]
                    duration = int(now - data['last_update'])
                    
                    if duration > 0:
                        asyncio.create_task(storage.member_stats.increment(user_id, guild_id, "voice_all_time", duration))
                        asyncio.create_task(storage.member_stats.increment(user_id, guild_id, "voice_weekly", duration))
                        asyncio.create_task(storage.member_stats.increment(user_id, guild_id, "voice_daily", duration))

                        if data['muted']:
                            asyncio.create_task(storage.member_stats.increment(user_id, guild_id, "voice_muted_all_time", duration))
                            asyncio.create_task(storage.member_stats.increment(user_id, guild_id, "voice_muted_weekly", duration))
                            asyncio.create_task(storage.member_stats.increment(user_id, guild_id, "voice_muted_daily", duration))
                        if data['deafened']:
                            asyncio.create_task(storage.member_stats.increment(user_id, guild_id, "voice_deafened_all_time", duration))
                            asyncio.create_task(storage.member_stats.increment(user_id, guild_id, "voice_deafened_weekly", duration))
                            asyncio.create_task(storage.member_stats.increment(user_id, guild_id, "voice_deafened_daily", duration))
                        if data['afk']:
                            asyncio.create_task(storage.member_stats.increment(user_id, guild_id, "voice_afk_all_time", duration))
                            asyncio.create_task(storage.member_stats.increment(user_id, guild_id, "voice_afk_weekly", duration))
                            asyncio.create_task(storage.member_stats.increment(user_id, guild_id, "voice_afk_daily", duration))
                        
                        # logger.debug(f"Incremented {duration}s voice stats for {member.name}")

                    # Update state
                    data['last_update'] = now
                    data['muted'] = after.self_mute or after.mute
                    data['deafened'] = after.self_deaf or after.deaf
                    data['afk'] = after.afk

            # User left voice completely
            elif before.channel and key in self.voice_tracking:
                data = self.voice_tracking[key]
                duration = int(now - data['last_update'])
                ...
                if duration > 0:
                    ...
                    if data['afk']:
                        ...
                        asyncio.create_task(storage.member_stats.increment(user_id, guild_id, "voice_afk_weekly", duration))
                        asyncio.create_task(storage.member_stats.increment(user_id, guild_id, "voice_afk_daily", duration))
                    # logger.debug(f"Final {duration}s voice stats for {member.name} (left VC)")

                del self.voice_tracking[key]

        except Exception as e:
            logger.error(f"Error tracking voice stats: {e}")
