...
class message(commands.Cog):
    ...
    async def check_for_afk(self, message: discord.Message):
        ...
        try:
            ...
            if global_afk:
                ...
                embed = discord.Embed(
                    description=f"Welcome back! You are no longer Globally AFK {self.bot.emoji.SUCCESS}",
                    color=0x2b2d31
                )
            ...
            if guild_afk:
                ...
                embed = discord.Embed(
                    description=f"Welcome back! You are no longer AFK in this server {self.bot.emoji.SUCCESS}",
                    color=0x2b2d31
                )
    ...
    async def bot_mention_handler(self, message: discord.Message):
        ...
        # Improved mention detection: direct pings or pings with minimal extra whitespace/characters
        if self.bot.user.mentioned_in(message) and not message.mention_everyone:
            # We only respond if the message IS just the mention (ignoring potential reply context)
            ...
            if not clean_content:
                ...
                embed = discord.Embed(
                    description=(
                        f"**Hey {message.author.name}**, welcome to **{self.bot.user.name}** {self.bot.emoji.WAVE}\n"
                        f"My current prefix for this server is `{guild_prefix}`\n\n"
                        f"{self.bot.emoji.BOT} **Status Overview**\n"
                        f"> **Latency:** `{latency}ms`\n"
                        f"> **Members:** `{member_count}`\n\n"
                        f"I am a premium, high-performance music and utility bot designed to elevate your server experience. Join a voice channel and type `{guild_prefix}play` to begin, or use `{guild_prefix}help` to explore my elite feature suite."
                    ),
                    color=0x2b2d31
                )
                embed.set_footer(text=f"Toxic (7ox4) • Powered by {self.bot.user.name}", icon_url=self.bot.user.display_avatar.url)
                
                # Using the premium banner link
                embed.set_image(url="https://cdn.discordapp.com/attachments/1464913496537042995/1498229215919607918/Elara_Music_Banner.png")
                
                embed.set_thumbnail(url=self.bot.user.display_avatar.url)

                view = discord.ui.View()
                view.add_item(discord.ui.Button(style=discord.ButtonStyle.url, label="Invite", url=self.bot.urls.INVITE))
                view.add_item(discord.ui.Button(style=discord.ButtonStyle.url, label="Support", url=self.bot.urls.SUPPORT))
    ...
    async def check_automod(self, message: discord.Message):
        ...
        try:
            ...
            # Heat / Rate Limit Check
            heat_max = guild_cache.get("heat_max_messages", 5)
            heat_interval = guild_cache.get("heat_interval", 5)
            if heat_max > 0:
                user_id = message.author.id
                current_time = time.time()
                if current_time - self.user_message_timestamps[user_id] > heat_interval:
                    self.user_message_counts[user_id] = 0
                    self.user_message_timestamps[user_id] = current_time
                self.user_message_counts[user_id] += 1
                if self.user_message_counts[user_id] > heat_max:
                    await self.antispam_punishment(message, guild_cache, delete_limit=1)
                    return logger.info("Heat threshold exceeded")

            # Word Blacklist Check
            bad_words = json.loads(guild_cache.get("antibadwords_words", "[]"))
            if bad_words:
                content = message.content.lower()
                if any(word in content for word in bad_words):
                    await self.antispam_punishment(message, guild_cache, delete_limit=1)
                    return logger.info("Blacklisted word detected")
            ...
            # Global Ignores
            ignored_users = json.loads(guild_cache.get("ignored_users", "[]"))
            if message.author.id in ignored_users:
                return logger.info("User is ignored from automod")
            
            ignored_roles = json.loads(guild_cache.get("ignored_roles", "[]"))
            if any(role.id in ignored_roles for role in message.author.roles):
                return logger.info("User role is ignored from automod")

            ignored_channels = json.loads(guild_cache.get("ignored_channels", "[]"))
            if message.channel.id in ignored_channels:
                return logger.info("Channel is ignored from automod")
    ...
    async def on_message(self, message: discord.Message):
        ...
        # Track Member Stats
        try:
            asyncio.create_task(storage.member_stats.increment(message.author.id, message.guild.id, "messages_all_time"))
            asyncio.create_task(storage.member_stats.increment(message.author.id, message.guild.id, "messages_weekly"))
            asyncio.create_task(storage.member_stats.increment(message.author.id, message.guild.id, "messages_daily"))
        except:
            pass
        ...
        try:
            ...
            await self.bot_mention_handler(message)
        ...
        try:
            if await self.music_channel_module(message):
                return
        except:
            pass

        # IMPORTANT: This line allows the bot to process prefix commands!
