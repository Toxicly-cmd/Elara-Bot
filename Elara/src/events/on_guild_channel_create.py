...
class on_guild_channel_create(commands.Cog):
    ...
    async def anti_channel_create_module(self,channel:discord.abc.GuildChannel):
        ...
        try:
            ...
            if anti_nuke_cache.get('wall_on'):
                action = anti_nuke_cache.get('anti_channel_create_punishment', 'kick')
                # Force limit hit to trigger punishment
                self.create_channel_timeouts.setdefault(str(channel.guild.id), {}).setdefault(str(creator.id), {})['count'] = 999
                logger.info(f"Antinuke Wall: Triggering punishment for {creator} in {channel.guild.name}")
...
async def setup(bot):
    await bot.add_cog(on_guild_channel_create(bot))
