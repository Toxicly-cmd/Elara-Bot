...
from Elara.config.config import users as users_config
...
from Elara.src.startup import giveaways
from Elara.src.startup import j2c_controller
class Wavelink(commands.Cog):
    ...
    async def on_wavelink_track_exception(self, payload: wavelink.TrackExceptionEventPayload):
        guild_name = payload.player.guild.name if payload.player and payload.player.guild else "Unknown Server"
    ...
    async def on_wavelink_player_update(self, payload: wavelink.PlayerUpdateEventPayload):
        try:
            ...
            if last_refresh and (now - last_refresh).total_seconds() < 15:
                return
