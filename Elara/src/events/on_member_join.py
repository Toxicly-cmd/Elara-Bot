...
class on_member_join(commands.Cog):
    ...
    async def panic_mode_module(self, member: discord.Member):
        try:
            panic_cache = cache.panicmode.get(str(member.guild.id))
            if not panic_cache or not panic_cache.get('enabled'):
                return
            
            action = panic_cache.get('punishment', 'kick')
            reason = "Panic Mode Enabled"
            ...
            logger.info(f"Panic Mode: {action}ed {member} in {member.guild.name}")
        except Exception as e:
            logger.error(f"Error in panic_mode_module: {e}")
    ...
    async def on_member_join(self, member: discord.Member):
        try:
            asyncio.create_task(self.panic_mode_module(member))
        except:
            pass
        try:
