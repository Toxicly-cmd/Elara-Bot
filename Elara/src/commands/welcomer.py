...
from Elara.src.checks.variables import fetch_variables
...
class Welcomer(commands.Cog):
    ...
    async def welcomer(self, ctx):
        ...
        try:
            ...
            embed.description = "**__Welcomer__**\n"
    ...
    async def welcome(self, ctx: commands.Context):
        ...
        try:
            ...
            embed = discord.Embed(
                title=f"{self.bot.emoji.WELCOME} Welcome Configuration",
                description=(
                    "Configure your automated greeting systems.\n\n"
                    "**__Commands:__**\n"
                ),
                color=0x2b2d31
            )
        ...
        try:
            ...
            async def get_embed():
                try:
                    ...
                    status = self.bot.emoji.ENABLED if welcomer_cache.get('welcome') else self.bot.emoji.DISABLED
                    ...
                    elif welcomer_cache.get("welcome_message"):
                        welcome_type = "Message"
                    elif welcomer_cache.get("welcome_embed"):
                        welcome_type = "Embed"
                    else:
                        welcome_type = "Not Set"

                    embed = discord.Embed(
                        description=f"**Welcome Module Settings** {self.bot.emoji.WELCOMER}\n"
                        f"> **Status:** {status}\n"
                        f"> **Channel:** {f'<#{channel_id}>' if channel_id else '`Not Set`'}\n"
                        f"> **Type:** `{welcome_type}`\n\n"
                        f"**__Message Content__**\n"
                        f"```{welcomer_cache.get('welcome_message_content') or 'No message set'}```\n\n"
                        f"**__Embed Configuration__**\n"
                        f"> **Title:** `{welcomer_cache.get('welcome_embed_title') or 'None'}`\n"
                        f"> **Thumbnail:** {'`Set`' if welcomer_cache.get('welcome_embed_thumbnail') else '`None`'}\n"
                        f"> **Image:** {'`Set`' if welcomer_cache.get('welcome_embed_image') else '`None`'}\n"
                        f"> **Footer:** `{welcomer_cache.get('welcome_embed_footer') or 'None'}`",
                        color=0x2b2d31
                    )
                    embed.set_footer(text=f"Toxic (7ox4) • Action by @{ctx.author.name}", icon_url=self.bot.user.display_avatar.url)
                    return embed
            ...
            async def preview_button_callback(interaction: discord.Interaction):
                ...
                try:
                    ...
                    if ctx.author.id != interaction.user.id:
                        ...
                        return await self.send_denied_embed(interaction, "You can't interact with this button")
        ...
        try:
            ...
            async def get_embed():
                try:
                    ...
                    logger.error(f"Error in get_embed: {e}")
                    return None
