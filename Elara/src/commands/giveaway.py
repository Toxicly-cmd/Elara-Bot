...
def parse_duration_to_seconds(duration: str) -> Union[int, None]:
    if not duration:
        return None
...
class Giveaway(commands.Cog):
    ...
    async def giveaway_command(self, ctx: commands.Context):
        ...
        try:
            ...
            if app_command and app_command.options:
                for sub in app_command.options:
                    formatted_cmds.append(f"`{app_command.name} {sub.name}`")
            
            embed.description = "**__Giveaway__**\n"
    ...
    async def giveaway_create(
        ...
        if not ctx.interaction:
            ...
            return await self.send_denied_embed(ctx, "This command can only be used as a slash command", delete_after=5)
        ...
        try:
            ...
            if ctx.interaction:
                ...
                if not message:
                    ...
                    return await self.send_denied_embed(ctx, "Something went wrong while creating the giveaway", delete_after=5)
        ...
        try:
            ...
            async def get_layout():
                ...
                section = ui.Section(
                    ...
                    ui.TextDisplay(f"**Ends:** <t:{int(ends_at.timestamp())}:R>\n**Host:** <@{host_id}>"),
                ...
                container.add_item(ui.TextDisplay(f"-# ID: {giveaway_id}"))
                    ...
                    async def exit_button_callback(interaction: discord.Interaction):
                        ...
                        try:
                            ...
                            if interaction.user.id != user.id:
                                ...
                                return await self.send_denied_embed(interaction, "You are not allowed to use this button")
        ...
        try:
            ...
            if not data:
                ...
                return await self._edit_message_with_optional_delete(
                    message,
                    embed=discord.Embed(
                        description=f"Giveaway {giveaway_id} not found", color=0x2b2d31
                    ),
                    delete_after=30,
                )
        ...
        try:
            ...
            if not all_giveaways:
                ...
                return await self._edit_message_with_optional_delete(
                    message,
                    embed=discord.Embed(
                        description="No giveaways found", color=0x2b2d31
                    ),
                    delete_after=30,
                )
