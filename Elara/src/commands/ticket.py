...
class Ticket(commands.Cog):
    ...
    def _embed_to_content(self, embed: discord.Union[Embed, None]) -> str:
        if not embed:
            return ""
        lines = []
        if embed.title:
        ...
        try:
            ...
            async def get_embed():
                try:
                    ...
                    embed = discord.Embed(
                        description=f"**Ticket Setup** {self.bot.emoji.TICKET}\nConfigure the multi-channel support ticket system for your server.\n\n",
                        color=0x2b2d31
                    )
            ...
            async def create_button_callback(interaction:discord.Interaction):
                try:
                    if interaction.user.id != ctx.author.id:
                        return await self.send_denied_embed(interaction, "You are not authorized to use this button")
                    ...
                    if len(ticket_settings_cache) >= ticket_module_limit:
                        return await self.send_denied_embed(interaction, f"You have reached the limit of {ticket_module_limit} ticket modules")
            ...
            async def edit_select_callback(interaction:discord.Interaction):
                try:
                    ...
                    if not selected_values:
                        return await interaction.response.send_message(embed=discord.Embed(description="No ticket module selected",color=0x2b2d31),ephemeral=True,delete_after=10)
                    ...
                    if not ticket_settings_data:
                        return await self.send_denied_embed(interaction, "Ticket module not found")
                    ...
                    async def get_selected_edit_embed():
                        ...
                        embed = discord.Embed(
                            description=f"**Edit Ticket Module** {self.bot.emoji.SETTINGS}\n"
                            f"**Module ID:** `{MODULE_ID}`\n"
                            "Modify the core settings for this specific ticket instance.",
                            color=0x2b2d31
                        )
                        embed.add_field(
                            name="Enabled",
                        ...
                        desc = f"**Category Open**: <#{ticket_settings_data.get('open_ticket_category_id')}>\n" if ticket_settings_data.get('open_ticket_category_id') else "**Category Open**: `Not Set`\n"
                        desc += f"**Category Closed**: <#{ticket_settings_data.get('closed_ticket_category_id')}>\n" if ticket_settings_data.get('closed_ticket_category_id') else "**Category Closed**: `Not Set`\n"
                        desc += f"**Channel**: <#{ticket_settings_data.get('ticket_panel_channel_id')}>\n" if ticket_settings_data.get('ticket_panel_channel_id') else "**Channel**: `Not Set`\n"
                        
                        roles = _support_roles(ticket_settings_data.get('support_roles','[]'))
                        desc += f"**Support Roles**: {', '.join([f'<@&{r}>' for r in roles]) if roles else '`Not Set`'}\n"
                        desc += f"**Created**: <t:{int(ticket_settings_data.get('created_at').timestamp())}:R>"
                    ...
                    async def get_selected_view(disabled:bool=False, summary_text:str=""):
                        ...
                        enable_disable_button = discord.ui.Button(
                            ...
                            style=discord.ButtonStyle.gray if not ticket_settings_data.get('enabled',False) else discord.ButtonStyle.gray,
                                ...
                                async def on_submit(self, interaction: discord.Interaction):
                                    try:
                                        ...
                                        if new_ticket_limit < 1:
                                            return await interaction.response.send_message(embed=discord.Embed(description="Ticket limit should be greater than 0",color=0x2b2d31),ephemeral=True,delete_after=10)
                                        ticket_settings_data = cache.ticket_settings.get(str(ctx.guild.id),{}).get(MODULE_ID,{})
                                        if not ticket_settings_data:
                                            return await self.send_denied_embed(interaction, "Ticket module not found")
                                        await interaction.response.defer()
    ...
    async def ticket_close(self,ctx: commands.Context,module_id:int=None,ticket_id:int=None):
        try:
            ...
            if not module_id and not ticket_id:
                ...
                if not ticket_data:
                    return await self._safe_ctx_send(ctx, embed=discord.Embed(description="This is not a ticket channel",color=0x2b2d31))
            else:
                ticket_data = await storage.tickets.get(guild_id=ctx.guild.id,ticket_module_id=module_id,ticket_id=ticket_id)
                if not ticket_data:
                    return await self._safe_ctx_send(ctx, embed=discord.Embed(description="Ticket not found",color=0x2b2d31))
            if ticket_data.get('closed',False):
                return await self._safe_ctx_send(ctx, embed=discord.Embed(description=f"Ticket: {ticket_data.get('ticket_id')} is already closed",color=0x2b2d31))
            if await ticket_panel.ticket_close_action(guild=ctx.guild,ticket_data=ticket_data,bot=self.bot,closed_by=ctx.author):
                await self._safe_ctx_send(ctx, embed=discord.Embed(description="Ticket closed successfully",color=0x2b2d31))
            else:
                await self._safe_ctx_send(ctx, embed=discord.Embed(description="Failed to close the ticket",color=0x2b2d31))
        except Exception as e:
            logger.error(f"Error in file {__file__} at line {traceback.extract_tb(sys.exc_info()[2])[0][1]}: {e}")
    ...
    async def ticket_delete(self,ctx: commands.Context,module_id:int=None,ticket_id:int=None):
        try:
            ...
            if not ticket_data.get('closed',False):
                return await self._safe_ctx_send(ctx, embed=discord.Embed(description=f"Ticket: {ticket_data.get('ticket_id')} is not closed yet",color=0x2b2d31))
