...
from Elara.workflows.cache import load_cache, users_cache, guilds_cache
from Elara.utils.time import parse_duration
from datetime import datetime, timedelta


from Elara.console.generator import generate_redeem_code


from Elara.utils.generate import generate_directory_tree_string_split_text
...
from Elara.config.config import Types


from Elara.style import emoji


redeem_code_types = Types.redeem_code_types


from Elara.workflows.actions import change_user_subscription
...
class Root(commands.Cog):
    ...
    @checks.is_bot_extra_owner()
    async def root(self, ctx: commands.Context):

        try:

            embed = discord.Embed(
                description=f"**Root System** {self.bot.emoji.BOT}\nAccess privileged administrative protocols and core system configuration.\n\n**__Commands:__**\n",
                color=0x2b2d31,
            )
    ...
    @root.group(name="npr", help="Manage No Prefix Request (NPR) users", invoke_without_command=True)
    @checks.is_bot_extra_owner()
    async def npr(self, ctx: commands.Context):
        embed = discord.Embed(
            description=f"**NPR Management** {self.bot.emoji.BOT}\nManage user permissions for bypassing standard command prefix requirements.\n\n"
                f"> `{ctx.prefix}root npr add <user> [time]` : Grant NPR access\n"
                f"> `{ctx.prefix}root npr remove <user>` : Revoke NPR access\n"
                f"> `{ctx.prefix}root npr list` : View all NPR users",
            color=0x2b2d31
        )
        embed.set_footer(text=f"Toxic (7ox4) • Action by @{ctx.author.name}", icon_url=self.bot.user.display_avatar.url)
        await ctx.send(embed=embed)

    @npr.command(name="add", help="Grant NPR to a user with optional duration")
    @checks.is_bot_extra_owner()
    async def npr_add(self, ctx: commands.Context, user: discord.User, *, duration: str = None):
        is_owner = ctx.author.id in config.users.root
        
        if not is_owner and not duration:
            embed = discord.Embed(
                title="<:Warning:1498201154801238217> Duration Required",
                description=(
                    f"Please specify a duration for the NPR access!\n\n"
                    f"**Examples:**\n"
                    f" `14d` - 14 Days\n"
                    f" `2hr` - 2 Hours\n"
                    f" `30m` - 30 Minutes\n"
        ...
        if duration:
            ...
            if not time_delta:
                return await self.send_denied_embed(ctx, "Invalid duration format! Use `d`, `hr`, `m`, or `s`.")
            
            # 14 Day limit for Extra Owners
            if not is_owner and time_delta > timedelta(days=14):
                return await self.send_denied_embed(ctx, "Extra Owners can only grant a maximum of **14 days** of NPR at once.")
            
            user_data = await storage.users.get(user_id=user.id)
            current_end = None
            if user_data and user_data.get('no_prefix_end'):
                try:
                    # Handle stacking if there is already an expiration date
                    current_end = datetime.fromisoformat(user_data['no_prefix_end'])
            ...
            expire_at = (current_end + time_delta).isoformat()

        user_data = await storage.users.get(user_id=user.id)
        if not user_data:
            await storage.users.insert(user_id=user.id, no_prefix=True, no_prefix_end=expire_at)
        else:
            await storage.users.update(id=user_data['id'], no_prefix=True, no_prefix_end=expire_at)
        
        # Refresh Cache
        await users_cache.update(user.id)

        duration_text = f"for **{duration}**" if duration else "permanently"
        await self.send_success_embed(ctx, f"User {user.mention} has been updated to the **No prefix list** {duration_text}")

    @npr.command(name="remove", help="Revoke NPR from a user")
    @checks.is_bot_extra_owner()
    async def npr_remove(self, ctx: commands.Context, user: discord.User):
        user_data = await storage.users.get(user_id=user.id)
        if user_data:
            await storage.users.update(id=user_data['id'], no_prefix=False, no_prefix_end=None)
        
        # Refresh Cache
        await users_cache.update(user.id)

        await self.send_success_embed(ctx, f"User {user.mention} has been removed from the **No prefix list** permanently")

    @npr.command(name="list", help="List all NPR users")
    @checks.is_bot_extra_owner()
    async def npr_list(self, ctx: commands.Context):
        users = await storage.users.gets(no_prefix=True)
        if not users:
            return await ctx.send("No users found with NPR access.")
        
        user_list = []
        count = 0
        for u in users:
            expire_at = u.get('no_prefix_end')
            time_text = ""
            if expire_at:
                try:
                    dt = datetime.fromisoformat(expire_at)
                    if dt.astimezone() > datetime.now().astimezone():
                        time_text = f" | Expires: <t:{int(dt.timestamp())}:R>"
                    else:
                        continue # Skip expired
                except:
                    time_text = " | Invalid Date"
            else:
                time_text = " | **Permanent**"
                
            count += 1
            user_list.append(f"`{count}.` <@{u['user_id']}> (`{u['user_id']}`){time_text}")
        
        if not user_list:
            return await ctx.send("No active NPR users found.")
    ...
    @root.group(name="premium", help="Manage Premium status", invoke_without_command=True)
    @checks.is_bot_extra_owner()
    async def premium(self, ctx: commands.Context):
        embed = discord.Embed(
            description=f"**Premium Management** {self.bot.emoji.BOT}\nControl server-wide premium feature sets and subscription lifecycles.\n\n"
                f"> `{ctx.prefix}root premium guild add <guild_id> <type> [time]` : Activate premium\n"
                f"> `{ctx.prefix}root premium guild remove <guild_id>` : Deactivate premium",
            color=0x2b2d31
        )
        embed.set_footer(text=f"Toxic (7ox4) • Action by @{ctx.author.name}", icon_url=self.bot.user.display_avatar.url)
        await ctx.send(embed=embed)
    ...
    @premium_guild.command(name="add", help="Grant premium to a guild with optional duration")
    @checks.is_bot_extra_owner()
    async def premium_guild_add(self, ctx: commands.Context, guild_id: int, sub_type: str = "diamond_guild_premium", *, duration: str = None):
        is_owner = ctx.author.id in config.users.root
        ...
        if not is_owner and not duration:
            embed = discord.Embed(
                title="<:Warning:1498201154801238217> Duration Required",
                description=(
                    f"Please specify a duration for the Premium subscription!\n\n"
                    f"**Examples:**\n"
                    f" `14d` - 14 Days\n"
                    f" `2hr` - 2 Hours\n"
                    f" `30m` - 30 Minutes\n"
        ...
        if duration:
            ...
            if not is_owner and time_delta > timedelta(days=14):
                return await self.send_denied_embed(ctx, "Extra Owners can only grant a maximum of **14 days** of Premium at once.")
            
            guild_data = await storage.guilds.get(guild_id=guild_id)
            current_end = None
            if guild_data and guild_data.get('subscription_end'):
                try:
                    current_end = datetime.fromisoformat(guild_data['subscription_end'])
                    if current_end < datetime.now():
                        current_end = datetime.now()
                except:
                    current_end = datetime.now()
        ...
        guild_data = await storage.guilds.get(guild_id=guild_id)
        if not guild_data:
            await storage.guilds.insert(guild_id=guild_id, subscription=sub_type, subscription_end=expire_at)
        else:
            await storage.guilds.update(id=guild_data['id'], subscription=sub_type, subscription_end=expire_at)
        
        # Refresh Cache
        await guilds_cache.update(guild_id)
        
        duration_text = f"for **{duration}**" if duration else "permanently"
        await self.send_success_embed(ctx, f"Successfully granted **{sub_type.replace('_', ' ').title()}** to Guild ID: `{guild_id}` {duration_text}")

    @premium_guild.command(name="remove", help="Revoke premium from a guild")
    @checks.is_bot_extra_owner()
    async def premium_guild_remove(self, ctx: commands.Context, guild_id: int):
        guild_data = await storage.guilds.get(guild_id=guild_id)
        if guild_data:
            await storage.guilds.update(id=guild_data['id'], subscription="free")
            
        # Refresh Cache
        await guilds_cache.update(guild_id)
        
        await self.send_success_embed(ctx, f"Successfully revoked **Premium** from Guild ID: `{guild_id}`")

    @root.group(name="extraowner", help="Manage Bot Extra Owners", invoke_without_command=True)
    @checks.is_owner()
    async def extraowner(self, ctx: commands.Context):
        embed = discord.Embed(
            title=f"{self.bot.emoji.BOT} Extra Owner Management",
            description=(
                "Authorize privileged users with elevated administrative control.\n\n"
                f"> `{ctx.prefix}root extraowner add <user>` : Designate extra owner\n"
                f"> `{ctx.prefix}root extraowner remove <user>` : Rescind extra owner\n"
                f"> `{ctx.prefix}root extraowner list` : View authorized owners"
            ),
            color=0x2b2d31
        )
        embed.set_footer(text=f"Toxic (7ox4) • Action by @{ctx.author.name}", icon_url=self.bot.user.display_avatar.url)
    ...
    @extraowner.command(name="add", help="Grant Extra Owner status")
    @checks.is_owner()
    async def extraowner_add(self, ctx: commands.Context, user: discord.User):
        user_data = await storage.users.get(user_id=user.id)
        if not user_data:
            await storage.users.insert(user_id=user.id, extra_owner=True)
        else:
            await storage.users.update(id=user_data['id'], extra_owner=True)
        
        # Refresh Cache
        await users_cache.update(user.id)
        
        embed = discord.Embed(
            title=f"{self.bot.emoji.BOT} Extra Owner Updated",
            description=f"{self.bot.emoji.MENTIONABLE} User {user.mention} has been updated to the **Extra Owner list** permanently.",
            color=0x2b2d31
        )
        embed.set_thumbnail(url=self.bot.user.display_avatar.url)
        embed.set_footer(text=f"Action by @{ctx.author.display_name} | Elara", icon_url=self.bot.user.display_avatar.url)
        await ctx.send(embed=embed)

    @extraowner.command(name="remove", help="Revoke Extra Owner status")
    @checks.is_owner()
    async def extraowner_remove(self, ctx: commands.Context, user: discord.User):
        user_data = await storage.users.get(user_id=user.id)
        if user_data:
            await storage.users.update(id=user_data['id'], extra_owner=False)
        
        # Refresh Cache
        await users_cache.update(user.id)
        
        embed = discord.Embed(
            title=f"{self.bot.emoji.BOT} Extra Owner Revoked",
            description=f"{self.bot.emoji.DELETE} User {user.mention} has been removed from the **Extra Owner list** permanently.",
            color=0x2b2d31
        )
        embed.set_thumbnail(url=self.bot.user.display_avatar.url)
        embed.set_footer(text=f"Action by @{ctx.author.display_name} | Elara", icon_url=self.bot.user.display_avatar.url)
    ...
    @extraowner.command(name="list", help="List all Extra Owners")
    @checks.is_owner()
    async def extraowner_list(self, ctx: commands.Context):
        users = await storage.users.gets(extra_owner=True)
        if not users:
            return await ctx.send("No Extra Owners found.")
        
        user_list = []
        for i, u in enumerate(users, 1):
            user_list.append(f"`{i}.` <@{u['user_id']}> (`{u['user_id']}`)")
            
        embed = discord.Embed(
            title=f"{self.bot.emoji.BOT} Bot Extra Owners",
            description="\n".join(user_list),
            color=0x2b2d31
        )
        embed.set_thumbnail(url=self.bot.user.display_avatar.url)
        embed.set_footer(text=f"Total Extra Owners: {len(user_list)} | Elara", icon_url=self.bot.user.display_avatar.url)
        await ctx.send(embed=embed)

    @root.group(name="staff", help="Manage Bot Staff", invoke_without_command=True)
    @checks.is_bot_extra_owner()
    async def staff(self, ctx: commands.Context):
        embed = discord.Embed(
            description=f"**Staff Management** {self.bot.emoji.BOT}\nOversee the internal bot support and moderation team.\n\n"
                f"> `{ctx.prefix}root staff add <user>` : Appoint bot staff\n"
                f"> `{ctx.prefix}root staff remove <user>` : Discharge bot staff\n"
                f"> `{ctx.prefix}root staff list` : View active staff",
            color=0x2b2d31
        )
        embed.set_footer(text=f"Toxic (7ox4) • Action by @{ctx.author.name}", icon_url=self.bot.user.display_avatar.url)
        await ctx.send(embed=embed)

    @staff.command(name="add", help="Grant Staff status")
    @checks.is_bot_extra_owner()
    async def staff_add(self, ctx: commands.Context, user: discord.User):
        user_data = await storage.users.get(user_id=user.id)
        if not user_data:
            await storage.users.insert(user_id=user.id, staff=True)
        else:
            await storage.users.update(id=user_data['id'], staff=True)
        
        embed = discord.Embed(
            title="<a:premium_avon:1498201608985514125> Bot Staff Updated",
            description=f"<a:loading:1498201792574652507> User {user.mention} has been updated to the **Bot Staff list** permanently.",
            color=0x2b2d31
        )
        embed.set_footer(text=f"Toxic (7ox4) • Action by @{ctx.author.display_name}", icon_url=self.bot.user.display_avatar.url)
        await ctx.send(embed=embed)

    @staff.command(name="remove", help="Revoke Staff status")
    @checks.is_bot_extra_owner()
    async def staff_remove(self, ctx: commands.Context, user: discord.User):
        user_data = await storage.users.get(user_id=user.id)
        if user_data:
            await storage.users.update(id=user_data['id'], staff=False)
        
        embed = discord.Embed(
            title="<a:premium_avon:1498201608985514125> Bot Staff Revoked",
            description=f"<a:loading:1498201792574652507> User {user.mention} has been removed from the **Bot Staff list** permanently.",
            color=0x2b2d31
        )
        embed.set_footer(text=f"Toxic (7ox4) • Action by @{ctx.author.display_name}", icon_url=self.bot.user.display_avatar.url)
        await ctx.send(embed=embed)

    @staff.command(name="list", help="List all Bot Staff")
    @checks.is_bot_extra_owner()
    async def staff_list(self, ctx: commands.Context):
        users = await storage.users.gets(staff=True)
        if not users:
            return await ctx.send("No Bot Staff found.")
        
        user_list = [f"<@{u['user_id']}> (`{u['user_id']}`)" for u in users]
        embed = discord.Embed(
            title="Bot Staff",
            description="\n".join(user_list),
            color=0x2b2d31
    ...
    async def reload(self, ctx: commands.Context, name: str = None):
        ...
        if not name:
            ...
            reloading_embed = discord.Embed(description=f"**Reloading Units** {self.bot.emoji.LOADING}\nUpdating and refreshing all internal source modules...", color=0x2b2d31)
            reloading_embed.set_footer(text=f"Toxic (7ox4) • Action by @{ctx.author.name}", icon_url=self.bot.user.display_avatar.url)
            ...
            await self.bot.reload_extension("Elara.src")
            ...
            reloading_embed.title = "Successfully Reloaded Elara Src"
            ...
            if name.lower() not in valid_types:
                ...
                return await self.send_denied_embed(ctx, f"Invalid Type: `{name}`. Valid Types: `{'/'.join(valid_types)}`.", delete_after=10)
    ...
    async def restart(self, ctx: commands.Context):
        ...
        confirmation_embed = discord.Embed(
            ...
            description=f"{self.bot.emoji.WARNING} Are you sure you want to restart the bot?",
            color=0x2b2d31
        )
        confirmation_embed.set_footer(text=f"Toxic (7ox4) • Action by @{ctx.author.name}", icon_url=self.bot.user.display_avatar.url)
        ...
        async def yes_button_callback(interaction: discord.Interaction):
            ...
            embed = discord.Embed(
                title="Restarted", description="Restarted the bot.", color=0x2b2d31
            )
        ...
        async def no_button_callback(interaction: discord.Interaction):
            ...
            embed = discord.Embed(
                title="Canceled", description="Canceled the restart.", color=0x2b2d31
            )
    ...
    async def shutdown(self, ctx: commands.Context):
        ...
        shutted_down_embed = discord.Embed(
            title="Shutting Down", description="Shutting down the bot.", color=0x2b2d31
        )
    ...
    async def root_ping(self, ctx: commands.Context):
        ...
        try:
            ...
            message = await ctx.send(
                embed=discord.Embed(
                    title="Pinging", description="Pinging...", color=0x2b2d31
                )
            )
            ...
            await ctx.send(
                embed=discord.Embed(
                    description=f"An Error occurred while pinging: {e}", color=0x2b2d31
                ),
                delete_after=10,
            )
    ...
    async def generate(self, ctx: commands.Context):
        ...
        try:
            ...
            async def get_embed():
                ...
                embed = discord.Embed(
                    title="Generate Redeem Code", description="", color=0x2b2d31
                )
    ...
    async def blacklist(self, ctx: commands.Context):
        ...
        try:
            ...
            embed = discord.Embed(
                description=(
                    f"**Blacklist System** {self.bot.emoji.BOT}\n"
                    f"Restrict specific users or guilds from utilizing bot services.\n\n"
                    "**__Commands:__**\n"
                ),
                color=0x2b2d31,
            )
    ...
    async def blacklist_user(self, ctx: commands.Context):
        ...
        try:
            ...
            embed = discord.Embed(
                description=f"**User Blacklist** {self.bot.emoji.BOT}\nManage global user restrictions for the bot.\n\n",
                color=0x2b2d31,
            )
            ...
            if hasattr(ctx.command, "commands"):
                ...
                for command in ctx.command.commands:
                    ...
                    embed.description += f"> `{self.bot.BotConfig.PREFIX}bl user {command.name}` : {command.help}\n"
    ...
    async def blacklist_user_add(self, ctx: commands.Context, user: discord.User):
        ...
        try:
            ...
            if str(user.id) in self.bot.cache.ban_data.get("users", {}):
                ...
                return await self.send_denied_embed(ctx, f"{user.mention} is already blacklisted.")

            await storage.ban_data.insert(
                user_id=user.id,
            )

            await self.send_success_embed(ctx, f"Successfully blacklisted {user.mention} {self.bot.emoji.BAN}")
    ...
    async def blacklist_user_remove(self, ctx: commands.Context, user: discord.User):
        ...
        try:
            ...
            if str(user.id) not in self.bot.cache.ban_data.get("users", {}):
                ...
                return await self.send_denied_embed(ctx, f"{user.mention} is not blacklisted.")

            await storage.ban_data.delete(
                user_id=user.id,
            )

            await self.send_success_embed(ctx, f"Successfully unblacklisted {user.mention} {self.bot.emoji.SUCCESS}")
        ...
        try:
            ...
            if not blacklisted_users:
                ...
                return await ctx.send(
                    embed=discord.Embed(
                        description="No users are blacklisted", color=0x2b2d31
                    ),
                    delete_after=10,
                )
            ...
            async def get_embed():
                ...
                embed = discord.Embed(
                    description=f"**Blacklisted Users** {self.bot.emoji.BAN}\nList of all globally restricted user accounts.\n\n",
                    color=0x2b2d31
                )

                users_text = ""
                for i, user_id in enumerate(blacklisted_users[current_page_index], 1):
                    users_text += f"> `{i + (current_page_index * 5)}.` <@{user_id}> (`{user_id}`)\n"
                
                embed.description += users_text

                embed.set_footer(
                    text=f"Page {current_page_index+1}/{len(blacklisted_users)} • Action by @{ctx.author.name}",
    ...
    async def blacklist_guild_add(self, ctx: commands.Context, guild: discord.Guild):
        ...
        try:
            ...
            await ctx.send(
                embed=discord.Embed(
                    description=f"Guild is blacklisted.", color=0x2b2d31
                )
            )
    ...
    async def blacklist_guild_remove(self, ctx: commands.Context, guild: discord.Guild):
        ...
        try:
            ...
            await ctx.send(
                embed=discord.Embed(
                    description=f"Guild is unblacklisted.", color=0x2b2d31
                )
            )
    ...
    async def blacklist_guild_list(self, ctx: commands.Context):
        ...
        try:
            ...
            if not blacklisted_guilds:
                ...
                return await ctx.send(
                    embed=discord.Embed(
                        description="No guilds are blacklisted", color=0x2b2d31
                    ),
                    delete_after=10,
                )
            ...
            async def get_embed():
                ...
                embed = discord.Embed(
                    title="Blacklisted Guilds", color=0x2b2d31
                )
    ...
    async def blacklist_list(self, ctx: commands.Context):
        ...
        try:
            ...
            if not blacklisted_users and not blacklisted_guilds:
                ...
                return await ctx.send(
                    embed=discord.Embed(
                        description="No users/guilds are blacklisted", color=0x2b2d31
                    ),
                    delete_after=10,
                )
            ...
            async def get_embed():
                ...
                embed = discord.Embed(
                    title="Blacklisted Users/Guilds", color=0x2b2d31
                )
    ...
    async def server(self, ctx: commands.Context, guild: discord.Guild):
        ...
        try:
            ...
            if not guild:
                ...
                return await ctx.send(
                    embed=discord.Embed(description="Guild not found", color=0x2b2d31),
                    delete_after=10,
                )
            ...
            embed = discord.Embed(description=f"**Server Diagnostics** {self.bot.emoji.GUILD}\nReviewing critical metadata and operational statistics for this guild.", color=0x2b2d31)
    ...
    async def servers(self, ctx: commands.Context):
        ...
        try:
            ...
            async def get_embed():
                ...
                embed = discord.Embed(
                    title=f"Total Servers ({len(guilds)})", color=0x2b2d31
                )
    ...
    async def leaveserver(self, ctx: commands.Context, guild_id: int):
        ...
        try:
            ...
            await ctx.send(
                embed=discord.Embed(
                    description=f"Left the server **{guild.name}**", color=0x2b2d31
                )
            )
    ...
    async def serverinvite(self, ctx: commands.Context, guild_id: int):
        ...
        try:
            ...
            await ctx.send(
                embed=discord.Embed(
                    description=f"**{guild.name}** Invite: {invite}", color=0x2b2d31
                )
            )
    ...
    async def emojis(self, ctx: commands.Context):
        ...
        try:
            ...
            for emoji_vars in sorted_emoji_vars:
                ...
                # name it like emoji - Union[emoji1, emoji] - Union[emoji2, emoji] - emoji3
                ...
                await ctx.send(
                    embed=discord.Embed(
                        description="\n".join(emoji_vars), color=0x2b2d31
                    )
                )
    ...
    #                     return await interaction.response.send_message(embed=discord.Embed(description="You can't interact with this message",color=0x2b2d31),ephemeral=True,delete_after=5)
    ...
    #                                 return await interaction.response.send_message(embed=discord.Embed(description="An item with that name already exists",color=0x2b2d31),ephemeral=True,delete_after=5)
    ...
    #                                 return await interaction.response.send_message(embed=discord.Embed(description="Invalid price",color=0x2b2d31),ephemeral=True,delete_after=5)
    ...
    #                             temp_message = await interaction.followup.send(embed=discord.Embed(description=f"Item {new_item_name} has been added",color=0x2b2d31),ephemeral=True)
    ...
    #                             await interaction.response.send_message(embed=discord.Embed(description="An error occured",color=0x2b2d31),ephemeral=True,delete_after=5)
    ...
    #                                 return await interaction.response.send_message(embed=discord.Embed(description="Item not found",color=0x2b2d31),ephemeral=True,delete_after=5)
    ...
    #                             temp_message = await interaction.followup.send(embed=discord.Embed(description=f"Item {item_id} has been removed",color=0x2b2d31),ephemeral=True)
    ...
    #                             temp_message = await interaction.followup.send(embed=discord.Embed(description=f"Item {new_item_name} has been updated",color=0x2b2d31),ephemeral=True)
    ...
    @commands.command(name="noprefixadd", aliases=["npadd", "npra"], hidden=True)
    @checks.is_bot_extra_owner()
    async def noprefixadd(self, ctx: commands.Context, user: discord.User, *, duration: str = None):
        is_owner = ctx.author.id in self.bot.config.users.root
    ...
    @commands.command(name="noprefixremove", aliases=["npremove", "nprem", "nprm"], hidden=True)
    @checks.is_owner()
