...
def check_is_owner_predicate(ctx):
    if ctx.author.id in config.users.root:
        return True
    return False
...
def check_is_bot_extra_owner_predicate(user):
    user_cache = cache.users.get(str(user.id), {})
    if user_cache.get('extra_owner', False) or user.id in config.users.root:
        return True
    return False

def is_bot_extra_owner():
    return commands.check(lambda ctx: check_is_bot_extra_owner_predicate(ctx.author))

def check_is_bot_staff_predicate(user):
    user_cache = cache.users.get(str(user.id), {})
    if user_cache.get('staff', False) or check_is_bot_extra_owner_predicate(user):
        return True
    return False

def is_bot_staff():
    return commands.check(lambda ctx: check_is_bot_staff_predicate(ctx.author))

def has_fun_access_predicate(ctx):
    if check_is_bot_staff_predicate(ctx.author):
        return True
    if ctx.author.guild_permissions.administrator:
        return True
    
    # Exclude specific roles from using Fun commands
    excluded_roles = ["horny", "gay", "lesbian"]
    if any(any(excl in role.name.lower() for excl in excluded_roles) for role in ctx.author.roles):
        return False
    
    # Check for Chat Ban
    if user_id := ctx.author.id:
        guild_id = str(ctx.guild.id)
        if user_id in cache.chat_bans.get(guild_id, []):
            return False
        
    return True

def has_fun_access():
    return commands.check(has_fun_access_predicate)
    ...
    try:
        ...
        if role_position_check:
            if ctx.author.top_role.position < ctx.guild.me.top_role.position:
                await ctx.send(embed=discord.Embed(description=f"{ctx.bot.emoji.ERROR} Your role is lower than me", color=0x2b2d31), delete_after=5)
                return False
        ...
        if notify:
            await ctx.send(embed=discord.Embed(description=f"{ctx.bot.emoji.ERROR} You don't have the required permissions", color=0x2b2d31), delete_after=5)
        return False
    except Exception as e:
        print(f"Error in file {__file__}: {traceback.format_exc()}")
        return False
...
async def close_ticket_permissions(user,guild:discord.Guild,creator_id:int,support_role_ids,notify=True):
    try:
        ...
        if any([role.id in support_role_ids for role in user.roles]):
            return True
        if notify:
            await user.send(embed=discord.Embed(description=f"{user.bot.emoji.ERROR} You don't have the required permissions to close this ticket", color=0x2b2d31))
        return False
    except Exception as e:
        print(f"Error in file {__file__}: {traceback.format_exc()}")
        return False
...
async def check_is_owner(ctx,notify=True):
    try:
        if ctx.author==ctx.guild.owner or check_is_owner_predicate(ctx) or await check_is_owner_raw(ctx.author,ctx.guild):
            return True
        if notify:
            await ctx.send(embed=discord.Embed(description=f"{ctx.bot.emoji.ERROR} You are not authorized to use this command", color=0x2b2d31), delete_after=10)
        return False
    except Exception as e:
        print(f"Error in file {__file__}: {traceback.format_exc()}")
        return False
...
async def check_if_user_can_manage_this_role(ctx:commands.Context,role:discord.Role):
    if ctx.guild.me.top_role.position <= role.position:
        await ctx.send(embed=discord.Embed(description=f"{ctx.bot.emoji.ERROR} The role is higher than me", color=0x2b2d31), delete_after=5)
        return False
    ...
    if ctx.author.top_role.position > role.position:
        return True
    await ctx.send(embed=discord.Embed(description=f"{ctx.bot.emoji.ERROR} You can't manage a role higher or equal to yours", color=0x2b2d31), delete_after=5)
    return False

async def check_if_user_can_manage_this_member(ctx:commands.Context,member:discord.Member):
    if ctx.guild.me.top_role.position <= member.top_role.position:
        await ctx.send(embed=discord.Embed(description=f"{ctx.bot.emoji.ERROR} The member role is higher than me", color=0x2b2d31), delete_after=5)
        return False
    if ctx.author == ctx.guild.owner:
        return True
    if ctx.guild.owner == member:
        await ctx.send(embed=discord.Embed(description=f"{ctx.bot.emoji.ERROR} You can't manage the server owner", color=0x2b2d31), delete_after=5)
        return False
    if ctx.author == member:
        return True
    if ctx.author.top_role.position > member.top_role.position:
        return True
    await ctx.send(embed=discord.Embed(description=f"{ctx.bot.emoji.ERROR} You can't manage a member with a higher or same role", color=0x2b2d31), delete_after=5)
    return False
