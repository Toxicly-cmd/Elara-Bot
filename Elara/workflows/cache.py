...
from storage import chat_bans as chat_bans_db
...
class ChatBans:
    ...
    async def initialize(self):
        cache.chat_bans = {}
        datas = await chat_bans_db.get_all()
        for data in datas:
            guild_id = str(data['guild_id'])
            if guild_id not in cache.chat_bans:
                cache.chat_bans[guild_id] = []
            cache.chat_bans[guild_id].append(data['user_id'])
        logger.database("Chat Bans cache loaded ✅")

    async def update(self, guild_id, user_id, data=None):
        if not data:
            data = await chat_bans_db.get(guild_id=guild_id, user_id=user_id)
        if not data: return
        guild_id = str(guild_id)
        if guild_id not in cache.chat_bans:
            cache.chat_bans[guild_id] = []
        if user_id not in cache.chat_bans[guild_id]:
            cache.chat_bans[guild_id].append(user_id)
    
    async def delete(self, guild_id, user_id):
        guild_id = str(guild_id)
        if guild_id in cache.chat_bans and user_id in cache.chat_bans[guild_id]:
            cache.chat_bans[guild_id].remove(user_id)

chat_bans_cache = ChatBans()
